...
func CreateProfile(profile model.Profile) error {
	return config.DB.Create(&profile).Error //insert into users (name, email) values (?, ?)
}

func UpdateProfile(profile model.Profile) error {
	return config.DB.Save(&profile).Error //update users set name = ?, email = ? where id = ?
}

func DeleteProfile(id int) error {
	return config.DB.Delete(&model.Profile{}, id).Error //delete from users where id = ?
}
